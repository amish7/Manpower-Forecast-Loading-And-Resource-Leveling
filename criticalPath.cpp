...
void takingInput(int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		string id;
		cout << "Please enter the ID " << i + 1 << " of the entity : ";
		cin >> id;
		val.name_of_entity = id;
		cout << "Please enter the duration time : ";
		cin >> du;
		val.duration_time = du;
		cout << "Please enter the number of the predecessors : ";
		...
		if (num_of_predecessors != 0) {
			for (int j = 0; j < num_of_predecessors; j++)
			{
				...
				//enter the entity into new arr
				for (int l = 0; l < size; l++)
				{
					if (Network[l].name_of_entity == ID_search)
					{
						find[f].name_of_entity = Network[l].name_of_entity;
			...
			Network[i].name_of_entity = val.name_of_entity;
...
void calculatingTime(int size)
{
	...
	for (int i = size - 2; i >= 0; i--)
	{
		...
		for (int j = i; j < size; j++)
		{
			...
			{
				if (Network[i].name_of_entity == Network[j].depend[w])
				{
					cheak = false;
					break;
				}
		...
		else
		{
			...
			for (int j = i; j < size; j++)
			{
				for (int w = 0; w < Network[j].num_of_predecessors_c; w++)
				{
					if (Network[i].name_of_entity == Network[j].depend[w])
					{
						find1[f].name_of_entity = Network[j].name_of_entity;
...
void finsh(int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		{
			answer += Network[i].name_of_entity;
			answer += " ";
		}
	}
...
{
	for (int i = 0; i < size; i++) {
		cout << "ID of the entity : " << Network[i].name_of_entity << endl;
		cout << "The duration time : " << Network[i].duration_time << endl;
		cout << "Early Start : " << Network[i].Est << endl;
		cout << "Early Finish : " << Network[i].Efi << endl;
		cout << "Late Start : " << Network[i].Lst << endl;
		cout << "Late Finish : " << Network[i].Lfi << endl;
